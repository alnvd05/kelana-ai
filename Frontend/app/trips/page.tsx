      ...
      <div className="mx-auto w-full max-w-6xl">
        <header className="flex flex-col gap-6 border-b border-[#c79a44]/30 pb-8 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">Trip History</h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-[#f6eedd]/65 sm:text-base">
              Revisit every itinerary saved in PostgreSQL without generating another AI response.
            </p>
          </div>
          <Link
            href="/"
        ...
        <section className="pt-8" aria-live="polite">
          {!isLoading && !error && trips.length > 0 && (
            <>
              ...
              <p className="mb-5 text-sm text-[#f6eedd]/60">
                Showing {matchingTrips.length} of {trips.length} saved {trips.length === 1 ? "itinerary" : "itineraries"}
              </p>
            </>
          )}
          ...
          {!isLoading && error && (
            <div className="rounded-3xl border border-[#d16850]/40 bg-[#4b211d]/40 p-8">
              <h2 className="text-xl font-black">We could not load your trips.</h2>
          ...
          {!isLoading && !error && trips.length === 0 && (
            <div className="rounded-3xl border border-[#c79a44]/30 bg-[#0e2a2c] p-10 text-center">
              ...
              <h2 className="mt-5 text-2xl font-black">No trips found.</h2>
              <p className="mt-2 text-[#f6eedd]/65">Create your first itinerary to start your history.</p>
              <Link
                href="/"
          ...
          {!isLoading && !error && trips.length > 0 && matchingTrips.length === 0 && (
            <div className="rounded-3xl border border-dashed border-[#c79a44]/50 bg-[#0e2a2c] p-10 text-center">
              ...
              <h2 className="mt-4 text-2xl font-black">No matching trips.</h2>
              <p className="mt-2 text-[#f6eedd]/65">Try another destination or travel style.</p>
              <button
                type="button"
                onClick={() => {
                  setSearchTerm("");
